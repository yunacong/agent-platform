# workflow_params.md（Day8：Parameter Extractor）
...
- default: `all`
- mapping:
  - 按计划/活动/campaign → `campaign`
  - 按素材/创意/creative → `creative`
  - 按人群/定向/audience → `audience`
  - 按地域/地区/geo → `geo`
  - 按设备/device → `device`
  - 没提到 → `all`
...
- default: `debug_drop`
- mapping:
  - “想提ROI/提升ROAS/提回报” → `raise_roi`
  - “降CPA/降成本/成本变贵” → `lower_cpa`
  - “稳量/放量/跑量” → `stabilize_delivery`
  - “掉了/下降/异常/排查原因/为什么变差” → `debug_drop`
  - “提CTR/点击率太低” → `improve_ctr`
  - “提CVR/转化率太低” → `improve_cvr`

---

## 2) Parameter Extractor 指令（Prompt）
...
- 出现“花费/消耗/预算/spend” -> metric=SPEND（goal 若未说明保持默认）
...
【默认值】
date_range 默认 近7天；metric 默认 ROI；dimension 默认 all；goal 默认 debug_drop
